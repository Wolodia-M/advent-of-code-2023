...
int main(int argc, char **argv) {
    ...
    case SEEDS: {
      ...
      while (std::getline(line_stream, line1, ' ')) {
        if (std::isdigit(line1[0])) {
          seeds.push_back(std::strtoul(line1.c_str(), NULL, 10));
        }
      }
    } break;
    case SEED_TO_SOIL: {
  ...
  // Perform mappings
  std::vector<unsigned long int> locations;
  for (auto seed : seeds) {
    ...
    unsigned long int water = perform_map(fertilize, &fertilize_to_water_map);
