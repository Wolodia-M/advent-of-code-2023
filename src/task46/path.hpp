...
private:
  ...
  void pathfind(Position start, Position end, Path current, int x, int y,
    ...
    auto valid_next = [&x, &y, this](int new_x, int new_y) -> bool {
      ...
      if (this->map[this->idx(new_x, new_y)] == Tile::SLOPE_EAST) {
        return true;
      }
      if (this->map[this->idx(new_x, new_y)] == Tile::SLOPE_WEST) {
        return true;
      }
      if (this->map[this->idx(new_x, new_y)] == Tile::SLOPE_SOUTH) {
        return true;
      }
      if (this->map[this->idx(new_x, new_y)] == Tile::SLOPE_NORTH) {
        return true;
      }
      return false;
    };
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (valid(new_x, new_y) &&
        ...
        printf("found size: %lu, current_size: %lu, x: %d, y: %d\n",
               store->size(), current.size(), new_x, new_y);
